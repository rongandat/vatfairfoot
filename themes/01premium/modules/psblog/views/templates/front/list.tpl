...
<div class="breadcrumb">
	<a title="{l s='Back home' mod='psblog'}" href="{$base_dir}">{l s='Home' mod='psblog'}</a><span class="navigation-pipe">&gt;</span><span class="navigation_end"><a title="{l s='All posts' mod='psblog'}" href="{$listLink}">{l s='Blog' mod='psblog'}</a></span> 
    {if isset($post_category) && $post_category}&gt; {l s='Category' mod='psblog'} "{$post_category->name}" {/if}
</div>

{if isset($notfound) && $notfound == true}
    
    {if isset($post_category)}
        <p>{l s='This category was not found' mod='psblog'}</p>
    {else}
        <p>{l s='The page was not found' mod='psblog'}</p>
    {/if}   
...
{else}
    ...
    <li {if $smarty.foreach.publications.last} class="last_item" {elseif $smarty.foreach.publications.first} class="first_item" {/if}>
        ...
        <img src="{$img_path}list/{$post.default_img_name}" width="{$blog_conf.img_list_width}" alt="{$post.title}" />
        </a>
        </div>
        {/if}
