...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-right-column">
		...
		{if $have_image}
			<span id="view_full_size">
				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_01prem')}" {if $jqZoomEnabled}class="jqzoom" alt="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_01prem')}"{else} title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" {/if} id="bigpic"/>
				<span class="span_link_container"><span class="span_link">{l s='View full size'}</span></span>
			</span>
		{else}
			<span id="view_full_size">
		...
		<!-- thumbnails -->
		<div id="views_block" class="clearfix {if isset($images) && count($images) < 2}hidden{else}carousel{/if}">
		{if isset($images) && count($images) > 2}<a id="view_scroll_left" class="hidden" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}">{l s='Previous'}</a>{/if}
		<div id="thumbs_list">
			<ul id="thumbs_list_frame" class="{*if isset($images) && count($images) > 3} jq_carousel3{/if*}">
				{if isset($images)}
					{foreach from=$images item=image name=thumbnails}
					{assign var=imageIds value="`$product->id`-`$image.id_image`"}
					<li id="thumbnail_{$image.id_image}">
						<a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_01prem')}" rel="other-views" class="thickbox {if $smarty.foreach.thumbnails.first}shown{/if}" title="{$image.legend|htmlspecialchars}">
							<img id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'medium_01prem')}" alt="{$image.legend|htmlspecialchars}" />
						</a>
					</li>
					{/foreach}
				{/if}
		...
		{/if}
		{if isset($images) && count($images) > 1}<p id="wrapResetImages" class="resetimg clear" style="display: none;"><img src="{$img_dir}icon/cancel_11x13.gif" alt="{l s='Cancel'}" width="11" height="13"/> <a id="resetImages" href="{$link->getProductLink($product)}" onclick="$('span#wrapResetImages').hide('slow');return (false);">{l s='Display all pictures'}</a></p>{/if}
		<!-- usefull links-->
	...
	<div id="pb-left-column">
		<h1>{if isset($product->manufacturer_name) && $product->manufacturer_name != ""}<span>{$product->manufacturer_name}</span> {/if}{$product->name|escape:'htmlall':'UTF-8'}</h1>
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<div class="product_attributes">
				...
				{if isset($HOOK_PRODUCT_OOS) && $HOOK_PRODUCT_OOS}
				<!-- Out of stock hook -->
			...
			<div class="content_prices">
				...
				{if $product->specificPrice AND $product->specificPrice.reduction_type == 'percentage'}
					<span class="reduction" id="reduction_percent_display">-{$product->specificPrice.reduction*100}%</span>
				{/if}
						...
						{foreach from=$accessories item=accessory name=accessories_list}
							{if ($accessory.allow_oosp || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
								...
								<li class="ajax_block_product {if $smarty.foreach.accessories_list.first}first_item{elseif $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description">
									<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{$accessory.legend|escape:'htmlall':'UTF-8'}" class="product_image"><img src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'medium_01prem')}" alt="{$accessory.legend|escape:'htmlall':'UTF-8'}" /></a>
