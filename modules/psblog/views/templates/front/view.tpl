            ...
            <ul>     
                {if $post_images}
                    {foreach from=$post_images item=img name=images}
                        <li>{if $blog_conf.view_display_popin}<a href="{$img_path}{$img.img_name}" rel="post" class="postpopin" title="{$post->title}">{/if}<img src="{$img_path}thumb/{$img.img_name}" width="{$blog_conf.img_width}" alt="{$post->title}" />{if $blog_conf.view_display_popin}</a>{/if}</li>
                    {/foreach}
                {/if}
            </ul>
            <div class="clear"></div>
