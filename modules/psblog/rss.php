...
<?php 
        $list = BlogPost::listPosts(true,true,null,null,false,$id_category,null,null);
                
	foreach ($list AS $item)
	{	
                $post = new BlogPost($item['id_blog_post']);
		...
		$link = BlogPost::linkPost($post->id,$post->link_rewrite,$post->id_lang);
		
		echo "\t\t\t<link><![CDATA[".htmlspecialchars($link)."]]></link>\n";
