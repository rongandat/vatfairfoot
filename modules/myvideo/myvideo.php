...
class MyVideo extends Module {
    ...
    public function __construct() {
        ...
        $this->displayName = $this->l('My Video');
        $this->description = $this->l('Add a Video View on your page.');
    }

    public function install() {
        if (!parent::install())
            return false;
        if (!Configuration::updateValue('MYVIDEO_CONF', base64_encode(serialize(self::$default_values))))
            return false;
    ...
    public function uninstall() {
        $tab_id = Tab::getIdFromClassName("AdminVideos");
        if ($tab_id) {
            $tab = new Tab($tab_id);
            $tab->delete();
        }

        $tab_id = Tab::getIdFromClassName("AdminVideoCategory");
        if ($tab_id) {
            $tab = new Tab($tab_id);
            $tab->delete();
        }

        $tab_id = Tab::getIdFromClassName("AdminVideo");
        if ($tab_id) {
            $tab = new Tab($tab_id);
            $tab->delete();
        }
    ...
    public static function getPreferences() {
        if (is_null(self::$pref)) {
            $config = Configuration::get('MYVIDEO_CONF');
