...
<div id="best-sellers_block_center" class="block products_block">
	<h4>{l s='Top sellers' mod='blockbestsellers2'}</h4>
		...
		<div class="block_content">
			...
			{foreach from=$best_sellers item=product name=myLoop}
				<li style="border-bottom:0" class="ajax_block_product {if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.iteration%$nbItemsPerLine == 0}last_item_of_line{elseif $smarty.foreach.myLoop.iteration%$nbItemsPerLine == 1}clear{/if} {if $smarty.foreach.myLoop.iteration > ($smarty.foreach.myLoop.total - ($smarty.foreach.myLoop.total % $nbItemsPerLine))}last_line{/if}">
					...
					<div class="product_desc"><a href="{$product.link}" title="{l s='More' mod='blockbestsellers2'}">{$product.description_short|strip_tags|truncate:130:'...'}</a></div>
					...
					<div>
						...
						<a class="button" href="{$product.link}" title="{l s='View' mod='blockbestsellers2'}">{l s='View' mod='blockbestsellers2'}</a>
					</div>
				</li>
			{/foreach}
			</ul>
			<p class="clearfix" style="padding: 5px;"><a style="float:right;" href="{$link->getPageLink('best-sales')}" title="{l s='All best sellers' mod='blockbestsellers2'}" class="button_large">{l s='All best sellers' mod='blockbestsellers2'}</a></p>
		</div>
	{else}
